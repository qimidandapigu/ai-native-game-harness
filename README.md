...
如果人生能像《星露谷物语》一样就好了：玩家继续种田、钓鱼和探险，把资料、文档和网页交给游戏里的 NPC。AI Native Game Harness 让 NPC 听懂要求、调用生产力工具，并在完成后回到游戏里汇报。
...
玩家可以先和游戏里的小汤圆正常交流，再用语音让它查资料、准备 HTML、写文档或继续修改成果。对玩家来说，是 NPC 接过工作、持续完成并回来汇报；在实现层，独立的 `@qimidandapigu/dsh-work-orchestrator` 会创建或恢复原生 Worker DSH Session，因此同一套工作能力以后也能被其他 NPC、桌面宠物或陪伴角色复用。
